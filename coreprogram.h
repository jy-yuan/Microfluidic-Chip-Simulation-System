#ifndef COREPROGRAM_H
#define COREPROGRAM_H
...
//查询某点的入度
int checkindegree(int n)
{
    int count = 0;
    for (int i : nodes[n].elist)
    {
        if (edges[i].leng == 0)
            continue;
        if (edges[i].n2 == n)
...
//计算某点n相邻边的浓度
void calcudense(int n)
{
    if (nodes[n].elist.empty())
        return;
    vector<int> inedge;  //存入边编号
    vector<int> outedge; //存出边编号
    for (int i = 0; i < nodes[n].elist.size(); i++)
    {
        int tmpedge = nodes[n].elist[i];
        if (edges[tmpedge].leng == 0)
            continue;
        else
        {
            if (edges[tmpedge].n2 == n)
                inedge.push_back(tmpedge);
            else
                outedge.push_back(tmpedge);
        }
    }
    if (inedge.size() == 0)
    {
    ...
    else if (inedge.size() == 2 && outedge.size() == 2)
    {
    }
    else
    {
...
//函数功能：计算芯片所有管道的液体流速以及液体浓度
//参数含义：num，正方形网格的边长（即网格一行的节点数量，比如8X8的网格，一行有8个节点，num为8）；length，存储网格中每个管道的长度，若管道不存在用0表示；i1,i2,o1,o2,o3
//				分别表示两个输入管道与三个输出管道在第几列。
vector<double> caluconspeedndense(int num, vector<double> &length, int i1, int i2, int o1, int o2, int o3)
{
    rect.clear();
    n = num;
    EDGESUM = 2 * n * n - 2 * n + 5;
    ...
    edges[EDGESUM - 5].dense = 1;

    initrect();
    getans();

    int degreearray[NODESUM];
    stack<int> nodestack; //存的是点的编号
    for (int i = 0; i < NODESUM; i++)
    {
        degreearray[i] = checkindegree(i);
    }
    for (int i = 0; i < NODESUM; i++)
    {
        if (degreearray[i] == 0)
    ...
    {
        auto now = nodestack.top();
        nodestack.pop();
        if (!nodes[now].elist.empty())
        {
            vector<int> inedge;  //存入边编号
            vector<int> outedge; //存出边编号
            for (int i : nodes[now].elist)
            { //存出入边
                if (edges[i].leng == 0)
                    continue;
                else
            ...
            //计算出边浓度
            if (inedge.size() == 0 || outedge.size() == 0)
            { //无入边/出边
            }
            else if (inedge.size() == 2 && outedge.size() == 2)
            { //“最后一种情况”
                int a, b, c, d;
                if (inedge[0] >= n * (n - 1) && inedge[0] < 2 * n * (n - 1))
                {
                    a = inedge[0];
                    b = inedge[1];
                }
                ...
                if (outedge[0] >= n * (n - 1) && outedge[0] < 2 * n * (n - 1))
                {
                    c = outedge[0];
                    d = outedge[1];
                }
                ...
                if (edges[b].v >= edges[c].v)
                {
                    edges[c].dense = edges[b].dense;
                    double middlepara = edges[a].dense * edges[a].v + edges[b].dense * edges[b].v - edges[c].dense * edges[c].v;
                    edges[d].dense = middlepara / edges[d].v;
                }
                else
                {
                    edges[d].dense = edges[a].dense;
                    double middlepara = edges[a].dense * edges[a].v + edges[b].dense * edges[b].v - edges[d].dense * edges[d].v;
                    edges[c].dense = middlepara / edges[c].v;
                }
            }
            else
            { //其他情况
                double invc_sum = 0, outv_sum = 0;
                for (int i : inedge)
                    invc_sum += edges[i].v * edges[i].dense;
                for (int i : outedge)
                    outv_sum += edges[i].v;
                for (int i : outedge)
                {
                    ...
                    {
                        edges[i].dense = invc_sum / outv_sum;
                    }
                }
            }
            //度数组减度,并入栈
            for (int i : outedge)
            {
                degreearray[edges[i].n2]--;
                if (degreearray[edges[i].n2] == 0)
                    nodestack.push(edges[i].n2);
            }
        }
    }
    ...
    for (int i = 0; i < EDGESUM; i++)
    {
        //cout << "edge " << i << " speed:" << edges[i].v << " concentration:" << edges[i].dense << endl;
    }
    ...
    v[3] = edges[EDGESUM - 3].dense;
    v[4] = edges[EDGESUM - 2].dense;
    v[5] = edges[EDGESUM - 1].dense;
    return v;
}
